...
class InterSolver<Method, Node, Fact> {
    ...
    private void doSolve() {
        ...
        while (!workList.isEmpty()){
            ...
            tmp.copyFrom((CPFact)result.getOutFact(node));
            for(ICFGEdge<Node> edge : icfg.getInEdgesOf(node)){
                Fact outFact = result.getOutFact(edge.getSource());//kong
                Fact fact = analysis.transferEdge(edge, outFact);
                analysis.meetInto(fact, result.getInFact(node));
            }
            analysis.transferNode(node,result.getInFact(node),result.getOutFact(node));
            if(!tmp.equals(result.getOutFact(node))) workList.add(node);
        }
