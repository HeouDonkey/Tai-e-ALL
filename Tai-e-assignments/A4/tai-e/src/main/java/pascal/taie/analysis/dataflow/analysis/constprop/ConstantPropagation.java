...
public class ConstantPropagation extends AbstractDataflowAnalysis<Stmt, CPFact> {
    ...
    public Value meetValue(Value v1, Value v2) {
        if (v1.isConstant()) {
            if (v2.isConstant()) {
                if (v2.getConstant() == v1.getConstant()) return Value.makeConstant(v1.getConstant());
                else if (v2.getConstant() != v1.getConstant()) return Value.getNAC();
            }
            else if (v2.isUndef()) return Value.makeConstant(v1.getConstant());
    ...
    public boolean transferNode(Stmt stmt, CPFact in, CPFact out) {
        if (stmt instanceof DefinitionStmt<?, ?>) {
            ...
            if (stmt.getDef().isPresent() && stmt.getDef().get() instanceof Var def) {//这里我不太理解，已经做了definitionstmt的判断了，但是如果不判断present依然会报错
                //tmp = in[b] -killb
                tmp.copyFrom(in);
